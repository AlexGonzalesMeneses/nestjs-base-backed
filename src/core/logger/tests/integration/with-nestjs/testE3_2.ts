...
export async function testE3_2() {
  const { error } = await peticionAxios({
    url: '/api/error/E3?variant=fetch',
  })
  ...
  expect(logEntry2.metadata.req).toHaveObj({
    method: 'GET',
    originalUrl: '/api/error/E3?variant=fetch',
    params: { codigo: 'E3' },
    query: { variant: 'fetch' },
    body: {},
  })
